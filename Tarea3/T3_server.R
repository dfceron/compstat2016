...
estims <- estims[sub, ]
  ...
  output$text <- renderText({
    paste("Se busca encontrar la relacion entre el salario anual de investigadores de una universidad
      (Y, en miles de dolares) y el indice de calidad de trabajo (X1), numero de 
      años de experiencia (X2) y el indice de exito en publicaciones (X3). La muestra consiste de 24 investigadores.",'<br/>','<br/>',"

    
      En las graficas de dispersion se observan que las variables explicativas presentan una relacion lineal
      con la variable de respuesta.",'<br/>','<br/>',"

      Se define el siguiente modelo de regresion lineal normal, 
      para explicar la relacion de las variables explicativas con el salario:",'<br/>','<br/>',"
  ...
  output$plot1 <- renderPlot(
    plot(salariosesc[,1],salariosesc[,2], xlab="Indice de Calidad de trabajo", ylab="Salario")
  
  )
  output$plot2 <- renderPlot(
   
    plot(salariosesc[,1],salariosesc[,3], xlab="Años de Experiencia", ylab="Salario")
  )
  output$plot3 <- renderPlot(
    
    plot(salariosesc[,1],salariosesc[,4], xlab="Indice de Exito en Publicaciones", ylab="Salario")
  )
