...
T3_ui <- function(id) {
  ns <- NS(id)
  tagList(
    box(title="Especificacion del Modelo", height = 560,status = "warning", solidHeader = TRUE,
        tabsetPanel(
          tabPanel("Modelo",htmlOutput(ns("text"))), 
          tabPanel("Datos",DT::dataTableOutput(ns("wideTable")))
        )),
    
    box(title="Graficas de Dispersion", height = 560,status = "success", solidHeader = TRUE,
    tabsetPanel(
    ...
    box(title="Graficas de Distribucion Inicial",status = "success", solidHeader = TRUE,
        tabsetPanel(
    ...
    box(title="Graficas de Distribucion Final",status = "success", solidHeader = TRUE,
        tabsetPanel(
